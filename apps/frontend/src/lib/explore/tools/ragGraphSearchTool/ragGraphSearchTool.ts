...
import { getNeogma, RELATIONSHIP_TYPES } from "@/lib/neo4j";
import {
  AchievementKeywords,
  CodeKeywords,
  EducationKeywords,
...
async function performVectorSearch(
  ...
  const cypher = `
      ...
      options.includeCode
		...
		OPTIONAL MATCH (e)-[:${RELATIONSHIP_TYPES.ACHIEVED}]->(a:Achievement)-[:${RELATIONSHIP_TYPES.DEMONSTRATES}]->(s:Skill)
...
function getPatternCypher(params: GraphSearchParams): string {
  ...
  return `
		MATCH (pat:Pattern {name: $patternName})<-[:${RELATIONSHIP_TYPES.IMPLEMENTS}]-(p:Project)
		OPTIONAL MATCH (p)-[:${RELATIONSHIP_TYPES.USES}]->(t:Technology)
		OPTIONAL MATCH (p)-[:${RELATIONSHIP_TYPES.DEMONSTRATES}]->(s:Skill)
