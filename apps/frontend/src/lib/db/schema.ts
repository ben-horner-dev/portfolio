...
export const humanEvaluations = evalSchema.table(
  "human_evaluations",
  {
    ...
    informationAccuracy: integer("information_accuracy").notNull(),
    responseHelpfulness: integer("response_helpfulness").notNull(),
    responseRelevance: integer("response_relevance").notNull(),
    responseCompleteness: integer("response_completeness").notNull(),
    conversationalQuality: integer("conversational_quality").notNull(),
  ...
  (table) => [
    ...
    check(
      "information_accuracy_must_be_1_to_5",
      sql`information_accuracy >= 1 AND information_accuracy <= 5`,
    ),
    check(
      "response_helpfulness_must_be_1_to_5",
      sql`response_helpfulness >= 1 AND response_helpfulness <= 5`,
    ),
    check(
      "response_relevance_must_be_1_to_5",
      sql`response_relevance >= 1 AND response_relevance <= 5`,
    ),
    check(
      "response_completeness_must_be_1_to_5",
      sql`response_completeness >= 1 AND response_completeness <= 5`,
    ),
    check(
      "conversational_quality_must_be_1_to_5",
      sql`conversational_quality >= 1 AND conversational_quality <= 5`,
    ),
    check(
